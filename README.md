...
To read the data into a dictionary for easy plotting. `QUANTITY_LABELS` holds the
LaTeX axis label registered for each quantity, so you can label plots straight from it:
```
import numpy as np
import matplotlib.pyplot as plt
from torch_tracker import Reader, QUANTITY_LABELS
...
for sim,l in enumerate(labels):
    ...
    order = np.argsort(t)
    y = r.data[l][quantity]
    plt.plot(t[order], y[order], label=l)

plt.xlabel(r"$t~[\mathrm{Myr}]$")
plt.ylabel(QUANTITY_LABELS[quantity])   # LaTeX label registered for the quantity
plt.legend()
plt.show()
```
...
The quantities below are registered in `torch_tracker/quantities.py` and can be listed in
the `quantities=[...]` argument of `TorchAnalysis`. 

<!-- BEGIN QUANTITIES TABLE (auto-generated by tools/generate_quantities_table.py; do not edit by hand) -->
...
| `sfe` | Computes the star formation efficiency defined as the total mass of stars over the total mass of gas. |
| `sfr` | Compute the star formation rate in Msun/yr using output intervals as dt. |
| `number_stars` | Computes the total number of stars. |
| `number_feedback_stars` | Computes the total number of feedback stars given the feedback mass defined in the torch_user.py pointed to in the python path. |
| `number_sinks` | Computes the number of sink particles. |
| `max_star_mass` | Returns the most massive star mass in Msun. |
...
>This table is auto-generated from the
registered quantities and their docstrings and is generated with `tools/generate_quantities_table.py`. This script gets executed on any new commits and the table is regenerated.
